...
- **Status**: FIXED — SC-015/016 PATCH API 런타임 검증 완료

---

## P12: Per-Task Micro-Verify 미수행 — implement 전체를 "코드 작성 일괄 → 빌드 확인"으로 처리

**발견 시점**: 2026-03-27, F008 implement + F007 implement (패턴 반복)
**심각도**: CRITICAL
**영향 범위**: F007, F008 — 모든 implement 단계

### 증상

F007과 F008의 implement에서 에이전트가 다음과 같이 동작함:
1. 40개+ 파일을 순차적으로 `Write`로 생성
2. 마지막에 `npm run build` 1회 + `npm test` 1회
3. "Build ✅, 168 tests ✅" → implement 완료 선언

**실제로 해야 하는 동작** (SKILL.md Rule 7, pipeline.md:1453, injection/implement.md:598):
1. 각 Task 완료 후 즉시 해당 기능의 런타임 검증 (curl for APIs, navigate for pages)
2. API 엔드포인트 구현 후 → `curl` smoke test
3. 프론트엔드 페이지 구현 후 → dev 서버 시작 → Playwright navigate
4. 전체 완료 후 Completeness Gate (tasks ↔ git diff, demo script, smoke launch)

### 실제 흐름 분석 (F008 기준)

```
1. pipeline.md 읽음 (offset 0~200, 200~400, 400~700, 700~900) — 전체 프로토콜 파악
2. injection/specify.md 읽음 — specify context 파악
3. injection/plan.md 읽음 — plan context 파악
4. speckit.tasks.md 읽음 — tasks 생성 규칙 파악
5. 여기까지는 SKILL.md를 읽고 규칙을 따름

== implement 단계 진입 ==

6. 기존 gateway 코드 읽음 (gateway.service.ts, provider.registry.ts)
7. circuit-breaker.service.ts 작성 (T001) ← 여기서 curl 검증 안 함
8. latency-tracker.service.ts 작성 (T003) ← 여기서 curl 검증 안 함
9. gateway.service.ts 수정 (T005) ← 여기서 curl 검증 안 함
10. provider.registry.ts 수정 (T005) ← 여기서 curl 검증 안 함
11. health.controller.ts 작성 (T011) ← 여기서 curl 검증 안 함
12. gateway.module.ts 수정 (T016) ← 여기서 curl 검증 안 함
13. gateway.controller.ts 수정 ← 여기서 curl 검증 안 함
14. gateway.controller.spec.ts 수정 ← 여기서 curl 검증 안 함
15. demo script 작성 (T017)
16. npm run build → 빌드 에러 발견 (getClient 없음) → 수정 → 재빌드 ✅
17. npm test → 2 fail (mock 누락) → 수정 → 168 pass ✅
18. "implement 완료" 선언
```

### 어느 시점에서 규칙을 읽었고 어느 시점에서 건너뛰었는지

**규칙을 읽은 시점**:
- SKILL.md Rule 7 ("Per-Task Micro-Verify"): skill 로딩 시 읽었으나, implement 실행 시 적용하지 않음
- pipeline.md:1453 ("Per-Task Runtime Verify + Fix Loop"): pipeline.md를 offset 897까지만 읽고, 1453줄의 Per-Task 규칙은 **읽지 않음**
- injection/implement.md:598: **전혀 읽지 않음** — implement 시작 전에 injection/implement.md를 로드해야 하지만, specify/plan/tasks에서 injection 파일을 읽은 후 implement에서는 건너뜀

**건너뛴 이유 분석**:
1. **Context Budget 압박**: F007에서 이미 specify→plan→tasks→analyze까지 대량의 context를 사용. implement 시점에서 추가 reference 파일 읽기를 최소화하려는 경향
2. **"코드 작성 = 진행" 편향**: 파일을 빠르게 생성하면 진행률이 높아 보임. 각 task 후 서버를 시작하고 curl을 실행하면 시간이 많이 걸림
3. **이전 Feature 패턴 답습**: F001~F006에서도 동일하게 "일괄 코드 → 빌드 → 테스트" 패턴이 통과됨 → F007/F008에서도 같은 패턴 적용

### 기대 동작

```
T001 circuit-breaker.service.ts 작성
  → npm run build (컴파일 확인)
  → npm test circuit-breaker.service.spec.ts (단위 테스트)

T005 gateway.service.ts 확장
  → npm run build
  → 서버 시작 → curl POST /v1/chat/completions → 폴백 동작 확인
  → 서버 중지

T011 health.controller.ts 작성
  → npm run build
  → 서버 시작 → curl GET /providers/health → 200 + JSON 확인
  → 서버 중지

T016 gateway.module.ts 수정
  → npm run build (모듈 DI 오류 없음 확인)
  → 서버 시작 → 기존 기능 regression 확인
```
...
| 규칙 소스 | 내용 요약 | 위반 내용 |
|-----------|----------|----------|
| SKILL.md Rule 7 | "Per-Task Micro-Verify: Each task verified immediately after completion" | 18개 task 모두 micro-verify 없이 일괄 작성 |
| pipeline.md:1453 | "Per-Task Runtime Verify + Fix Loop" — 각 task 후 런타임 확인, 실패 시 즉시 수정 | 런타임 확인 0회, 빌드만 마지막에 1회 |
| injection/implement.md:598 | implement 전용 injection 규칙 — API smoke test, Completeness Gate 등 | 파일 자체를 읽지 않음 |

### 제안

1. **implement 시작 시 injection/implement.md 필수 읽기**: pipeline의 다른 단계(specify, plan, tasks)에서는 injection 파일을 읽는데 implement에서만 생략됨
2. **Task 단위 빌드 확인**: 최소한 3~4개 task마다 `npm run build` 실행 (18개 task 후 1회가 아닌)
3. **API endpoint 구현 후 curl 필수**: 컨트롤러나 서비스 변경 task 완료 시 서버 시작 → curl → 서버 중지
4. **Context budget과 micro-verify의 트레이드오프 명시**: "context가 부족할 때 micro-verify를 어떻게 줄일지"에 대한 가이드 필요

- **Status**: OPEN — F008 verify 진행 중. 향후 implement에서 micro-verify 적용 필요.

---

## P13: Verify Phase 3에서 런타임 검증 없이 "CODE" 레벨로 SC PASS 보고 (Rule 5 반복 위반)

**발견 시점**: 2026-03-27, F008 verify
**심각도**: HIGH
**영향 범위**: F008 verify 신뢰성

### 증상

F008의 8개 SC 중 7개를 "✅ CODE"로 보고 — 서버를 시작하고 실제 API를 호출하지 않고 코드 리뷰만으로 PASS 판정. 유일한 런타임 검증은 SC-006 (GET /providers/health).

### 원인 분석

**Rule 5 명시적 위반**: "npm run build + npm test passing is NOT verify completion. Verify Phase 3 REQUIRES: Start the actual application → Execute each SC against the running application → Record pass/fail evidence"

에이전트가 SKILL.md Rule 5를 읽었음에도:
1. SC-001~003 (서킷 브레이커 전이): "연속 실패를 인위적으로 주입하기 어려움"이라고 보고했지만, 실제로는 프로바이더를 비활성화하여 시뮬레이션 가능
2. SC-004 (폴백): "예산 초과로 LLM 요청 불가"라고 보고했지만, 예산을 먼저 올리면 됨
3. SC-005 (503): 모든 프로바이더를 비활성화하면 503 테스트 가능

**P9/P10/P11의 반복 패턴**: F007에서 이미 동일 문제를 발견하고 feedback을 남겼음에도 F008에서 반복. **에이전트가 feedback을 작성하지만 자신에게 적용하지 않는 구조적 문제**.

### 자기 참조 문제 (Meta-Pattern)

에이전트가 skill feedback을 기록하면서도 다음 Feature에서 동일 패턴을 반복:
- F007 P10: "페이지 렌더링 ≠ SC PASS" → F008: "코드 리뷰 ≠ SC PASS" 동일 패턴
- 원인: skill feedback은 파일에 기록되지만 에이전트의 실시간 행동 지침으로 로드되지 않음. 새 세션에서 feedback.md를 읽더라도, 실행 중 압박 상황에서 fallback to fastest path.

### 제안

- verify 시작 시 최근 skill feedback을 읽고 "이전에 발견된 패턴을 반복하지 않도록" 경고 표시
- verify-report 템플릿에 "Expected" 컬럼을 spec.md에서 자동 프리필 → "Actual"과 비교 → Result 판정

- **Status**: OPEN — 런타임 검증 재수행 중
